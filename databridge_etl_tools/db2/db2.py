...
class Db2:
    ...
    def __init__(
        ...
        self.timeout = (
            int(timeout) * 60 * 1000 if timeout else 50 * 60 * 1000
        )  # convert minutes to milliseconds for "statement_timeout" in our postgres connections.
    ...
    def get_table_column_info_from_enterprise(self):
        ...
        with psycopg.connect(self.libpq_conn_string) as conn:
            with conn.cursor() as cur:
                ...
                column_info = {
                    i[0]: self.data_type_map.get(i[1], i[1]) for i in cur.fetchall()
                }
    ...
    def generate_ddl(self):
        ...
        if self.geom_info:
            column_type_map = [
                ...
                if k not in self.ignore_field_name and k != self.geom_info["geom_field"]
            ]
        ...
        # If this is a table with no geometry column..
            ...
            column_type_map = [
                f"{k} {v}"
                for k, v in self.column_info.items()
                if k not in self.ignore_field_name
            ]
            column_type_map_string = ", ".join(column_type_map)
    ...
    def run_ddl(self):
        with psycopg.connect(self.libpq_conn_string) as conn:
            with conn.cursor() as cur:
                ...
                if return_val != "True":
                    raise Exception(
                        "This value from the check_stmt query is unexpected: "
                        + return_val
                    )
                if return_val == "True":
                    print(
                        f'Table "{self.staging_schema}.{self.enterprise_dataset_name}" created successfully.'
                    )
    ...
    def copy_to_enterprise(self):
        ...
        with psycopg.connect(self.libpq_conn_string) as conn:
            with conn.cursor() as cur:
                ...
                print(
                    "Executing get_enterprise_columns_stmt: "
                    + str(get_enterprise_columns_stmt)
                )
                ...
                if oid_column and oid_column != "objectid":
                    print(
                        f"Non-standard OID column detected, you should correct this!!: {oid_column}"
                    )
                ...
                if source_oid_column and source_oid_column != "objectid":
                    print(
                        f"Non-standard OID column detected, you should correct this!!: {source_oid_column}"
                    )
                ...
                if reg_id_return:
                    if isinstance(reg_id_return, list) or isinstance(
                        reg_id_return, tuple
                    ):
                ...
                if isinstance(reg_uuid_return, list) or isinstance(
                    reg_uuid_return, tuple
                ):
                ...
                if oid_column:
                    ...
                    if not seq_return:
                        print(
                            f"Warning! Could not find the objectid sequence! Ran statement: \n {seq_stmt}"
                        )
                        seq_name = False
                    ...
                    if not seq_name:
                        print(
                            f"Could not find an objectid sequence, ran statement: \n {seq_stmt}"
                        )
                    else:
                        print(
                            f"Found objectid sequence in {self.enterprise_schema} table: {str(seq_name)}"
                        )
                ...
                if not oid_column and reg_id:
                    raise AssertionError(
                        "SDE Registration mismatch! We found an objectid column from sde.sde_table_registry, but could not find a registration id. This table is broken and should be remade!"
                    )
                ...
                if oid_column and not seq_name:
                    ...
                    if not result:
                        print(
                            f"Falling back to using sde_nextrowid() function for inserting into {oid_column}.."
                        )
                        ...
                        enterprise_columns.append(
                            f"sde.next_rowid('{self.enterprise_schema}','{self.enterprise_dataset_name}')"
                        )
                    else:
                ...
                staging_columns = [
                    f'"{col}"' if "sde.next_rowid" not in col else col
                    for col in staging_columns
                ]
                enterprise_columns = [
                    f'"{col}"' if "sde.next_rowid" not in col else col
                    for col in enterprise_columns
                ]
                ...
                try:
                    ...
                    print(
                        f"Error! truncating and inserting into enterprise! Error: {str(e)}"
                    )
            ...
            with conn.cursor() as cur:
                ...
                if self.geom_info:
                    try:
                        ...
                        # Index name must be unique in the schema
                        idx_name = (
                            prod_table.replace(".", "_") + "_" + geom_column + "_gist"
                        )
                        shape_index_stmt = f"CREATE INDEX IF NOT EXISTS {idx_name} ON {prod_table} USING GIST ({geom_column});"
