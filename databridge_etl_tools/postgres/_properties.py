...
def json_schema_s3_key(self):
    ...
    if self._json_schema_s3_key == None:
        self._json_schema_s3_key = self.s3_key.replace("staging", "schemas").replace(
            ".csv", ".json"
        )
...
def new_json_schema_s3_key(self):
    ...
    if not self._new_json_schema_s3_key:
        self._new_json_schema_s3_key = (
            self.s3_key.replace("staging/", "schemas/")
...
def export_json_schema(self):
    ...
    if self._export_json_schema is None:
        ...
        results = self.execute_sql(
            stmt, data=[self.table_name, self.table_schema], fetch="all"
        )
        ...
        """)
        results = self.execute_sql(
            pk_stmt, data=[self.table_name, self.table_schema], fetch="one"
        )
        ...
        if self.geom_field:
            ...
            """)
            results = self.execute_sql(
                stmt, data=[self.table_name, self.table_schema], fetch="one"
            )
            if results:
                srid = results[0]
            if not srid:
            ...
            if self.old_carto_format:
                ...
                if (
                    self.geom_type.lower() == "multipolygon"
                    or self.geom_type.lower() == "polygon"
                ):
            ...
            if (
                ...
                self.logger.info(
                    f"Detected bullshit geometry type in json schema for field {field['name']}, attempting to fix..."
                )
...
def primary_keys(self) -> "set":
    ...
    if self._primary_keys == None:
        ...
        """)
        results = self.execute_sql(
            stmt, data=[self.fully_qualified_table_name], fetch="all"
        )
...
def pk_constraint_name(self):
    ...
    if self._pk_constraint_name == None:
        ...
        """)
        results = self.execute_sql(
            constraint_stmt, data=[self.table_schema, self.table_name], fetch="all"
        )
...
def fields_and_types(self):
    ...
    if self._fields:
        ...
        # Funny method for getting the column names and data types for views
            ...
            self.database_object_type == "materialized_view"
            or self.database_object_type == "view"
        ):
            stmt = """
                select
...
def database_object_type(self):
    ...
    with self.conn.cursor() as cursor:
        ...
        if not res:
            raise AssertionError(
                f"Table doesnt appear to exist?: {self.table_schema}.{self.table_name}"
            )
        relkind = res[0]
    if type_map[relkind] in ["table", "materialized_view", "view"]:
        ...
        raise TypeError(
            """This database object is unsupported at this time.
...
def geom_type(self, value):
    if self.table_name == "testing" and self.table_schema == "test":
        ...
        check_table_stmt = (
            "SELECT EXISTS(SELECT * FROM pg_proc WHERE proname = 'geometry_type');"
        )
        result = self.execute_sql(check_table_stmt, fetch="one")[0]
        if result:
            geom_stmt = f"""
