...
class Postgres:
    ...
    from ._cleanup import check_remove_nulls, cleanup, vacuum_analyze
    from ._properties import (
        ...
        json_schema_path,
        json_schema_s3_key,
        new_json_schema_s3_key,
        pk_constraint_name,
        primary_keys,
        table_self_identifier,
        temp_csv_path,
    )
    from ._s3 import (
        get_csv_from_s3,
        get_json_schema_from_s3,
        load_csv_to_s3,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.s3_bucket and not self.s3_key:
            if self.local_json_schema_path is None:
                self.local_json_schema_path = (
                    f"/tmp/{self.table_schema}__{self.table_name}.json"
                )
        ...
        self.index_fields = (
            ...
            if kwargs.get("index_fields", None) is None
            or kwargs.get("index_fields", None).strip() == ""
    ...
    def __exit__(self, type, value, traceback):
        ...
        if type == None:  # No exception was raised before __exit__()
            try:
                ...
                self.logger.error(
                    "Workflow failed... rolling back database transactions.\n"
                )
                self.conn.rollback()
                raise e
            finally:
    ...
    def new_schema_from_s3(self, json_s3_key=None):
        ...
        if self._new_schema is None:
            ...
            self.logger.info(
                "Fetching json schema: s3://{}/{}".format(self.s3_bucket, json_s3_key)
            )

            s3 = boto3.resource("s3")
            try:
                s3.Object(self.s3_bucket, json_s3_key).download_file(
    ...
    def prepare_file(
        ...
        try:
            ...
            self.logger.info(
                "Exception encountered trying to load rows with utf-8 encoding, trying latin-1..."
            )
        ...
        if self.geom_field is not None and (self.geom_type in shape_types):
            ...
            rows = rows.addfield(
                "row_geom_type",
                lambda a: (
                    a[f"{self.geom_field}"].split("(")[0].split(";")[1].strip()
                    if a[f"{self.geom_field}"] and "(" in a[f"{self.geom_field}"]
                    else None
                ),
            )
            ...
            rows = rows.convert(
                self.geom_field,
                lambda u, row: (
                    u.replace(row.row_geom_type, "MULTI" + row.row_geom_type + " (")
                    + ")"
                    if "MULTI" not in row.row_geom_type
                    else u
                ),
                pass_row=True,
            )
            ...
            if force_2d(first_nonnull_shape) == first_nonnull_shape:
                self.logger.info(
                    "Source table is not M/Z enabled; no changes to shape data necessary"
                )
            else:
                ...
                if force2d:
                    ...
                    if zm_enabled_test:
                        self.logger.info(
                            "Target table is M/Z enabled. Keeping source shape data as-is..."
                        )
                    else:
                        self.logger.info(
                            "Target table is not M/Z enabled. Remving Z and/or M values from source shape data..."
                        )
                        rows = rows.convert(
                            self.geom_field,
                            lambda x: force_2d(x) if isinstance(x, str) else x,
                        )
        ...
        if mapping_dict != None:
            ...
            rows = rows.rename(
                {old: new for old, new in zip(header, str_header.split(", "))}
            )
        ...
        if self.check_exists(self.table_name, self.table_schema):
            for f_name, f_type, udt_type in self.fields_and_types:
                if udt_type.lower() == "_text" or f_type.lower() == "array":
                    self.logger.info(
                        f"Converting column {f_name} array type to have curly braces for postgres..."
                    )
                    rows = etl.convert(
                        rows, f_name, lambda v: v.replace("[", "{").replace("]", "}")
                    )
        # If we fail getting types directly from DB (first time loading the table and this tool hasn't made the table yet?), then get schema from S3 JSON
            ...
            for i, scheme in enumerate(schema):
                if scheme["type"] == "array":
                    print(
                        f"Converting column {scheme['name']} array type to have curly braces for postgres..."
                    )
                    rows = etl.convert(
                        rows,
                        scheme["name"],
    ...
    def create_table(self, rename_replace=False):
        with self.conn.cursor() as cursor:
            ...
            if self.check_exists(self.table_name, self.table_schema):
                ...
                database_field_dict = {
                    k: ("character varying" if v == "text" else v)
                    for k, v in database_field_dict.items()
                }
                schema_field_dict = {
                    k: ("character varying" if v == "text" else v)
            ...
            # Make the table if we have to (it doesn't exist or we dropped it in the lines before this)
            if (
                not self.check_exists(self.table_name, self.table_schema)
                or recreate_table
            ):
                ...
                if self.geom_field is None:
                    for s in schema:
                        if s["type"] == "geometry":
                            ...
                            self.logger.info(
                                f"Detected geometry field from json schema: {self.geom_field}\n"
                            )
                            break
                ...
                for i, scheme in enumerate(schema):
                    ...
                    if "constraints" in scheme.keys():
                        if (
                            "required" in scheme["constraints"].keys()
                            and scheme["constraints"]["required"]
                        ):
                            col += " NOT NULL"
                        else:
                            col += " NULL"
    ...
    def _is_registered(self) -> bool:
        ...
        with self.conn.cursor() as cursor:
            ...
            if registered_one:
                self.logger.info(
                    f"Found registration_id in sde.sde_table_registry: {registered_one}"
                )
            ...
            # clean up. Check out the postgres function public.force_drop_sde_table to see what tables to look at.
                ...
                raise AssertionError(
                    "Inconsistent registration detected! Please clean up SDE tables for this table."
                )
            else:
                return False
    ...
    def _is_m_or_z_enabled(self) -> bool:
        ...
        self.logger.info(
            "Checking for z or m values in sde.gdb_items.definition column.."
        )
        with self.conn.cursor() as cursor:
            ...
            if not result:
                self.logger.info(
                    "No XML file found sde.gdb_items definition col, this is unusual for a registered table!"
                )
                return False
            else:
                xml_def = result[0]
                if not xml_def:
                    ...
                    if not m_search:
                        self.logger.info(
                            "No <HasM> element found in xml definition, assuming False."
                        )
                    else:
                    ...
                    if not z_search:
                        self.logger.info(
                            "No <HasZ> element found in xml definition, assuming False."
                        )
                    else:
    ...
    def _make_mapping_dict(
        self, column_mappings: str = None, mappings_file: str = None
    ) -> dict:
        ...
        assert type(mapping_dict) == dict, (
            "Column mappings could not be read as a Python dictionary"
        )
        return mapping_dict
    ...
    def write_csv(
        ...
        with open(write_file, "r") as f:
            ...
            str_header = [
                "".join([char.replace("\ufeff", "") for char in colname])
                for colname in str_header
            ]
    ...
    FROM STDIN WITH (FORMAT csv, HEADER true)""").format(
                    table=table_identifier, cols_composed=cols_composed
                )
                self.logger.info(
                    f"copy_statement:{cursor.mogrify(copy_stmt).decode()}"
                )  # Does this mean they need to be correctly capitalized as identifiers?
        ...
        if self._is_registered():
            if self._registration_id:
                ...
                self.logger.info(
                    f"Updated objectid column in {self.table_schema}.i{self._registration_id} to {max_objectid + 1}.\n"
                )
            else:
                self.logger.info(
                    f"Table is registered, but no registration_id found. Cannot update objectid column.\n"
                )
        else:
            self.logger.info(
                f"Table is not registered, skipping objectid column update.\n"
            )
    ...
    def get_row_count(self) -> int:
        ...
        data = self.execute_sql(
            sql.SQL("SELECT count(*) FROM {}").format(self.table_self_identifier),
            fetch="many",
        )
        count = data[0][0]
        self.logger.info(
            f"{self.fully_qualified_table_name} current row count: {count:,}\n"
        )
        return count
    ...
    def extract(self, return_data: bool = False):
        ...
        * return_data (bool): If True return the data in memory as a 'geopetl.postgis.PostgisQuery'
        ...
        self.logger.info(
            "Note: petl can cause log messages to seemingly come out of order."
        )
        ...
        if self.with_srid is True:
            rows = etl.frompostgis(
                self.conn, self.fully_qualified_table_name, geom_with_srid=True
            )
        else:
            rows = etl.frompostgis(
                self.conn, self.fully_qualified_table_name, geom_with_srid=False
            )
        ...
        for field in self.fields_and_types:
            ...
            if "timestamp" in field[1].lower() and (
                "tz" not in field[1].lower()
                and "with time zone" not in field[1].lower()
            ):
        ...
        if timestamp_fields:
            ...
            rows_new = etl.convert(
                rows, timestamp_fields, pytz.timezone("US/Eastern").localize
            )
        ...
        if self.exclude_fields:
            self.logger.info(
                f"Excluding fields from extraction: {self.exclude_fields}\n"
            )
        ...
        try:
            ...
            self.logger.warning(
                "Exception encountered trying to extract to CSV with utf-8 encoding, trying latin-1..."
            )
        ...
        self.logger.info(
            f"Asserting counts match between current db count and extracted csv"
        )
        ...
        if self.s3_bucket and self.s3_key:
            ...
            os.replace(
                self.csv_path, os.path.join(os.getcwd(), self.table_name + ".csv")
            )
    ...
    def create_temp_table(self):
        ...
        with self.conn.cursor() as cursor:
            ...
            """).format(  # This is psycopg2.sql.SQL.format() not f string format
                    ...
                    ),  # Temp tables cannot be created in a user-defined schema
    ...
    def drop_table(self, schema_name: str, table_name: "str", exists="log"):
        ...
        if self.check_exists(table_name, schema_name):
            if exists == "error":
                raise ValueError(
                    f"Table {table_name} already exists and was set to be dropped."
                )
            if exists == "log":
    ...
    def truncate(self):
        ...
        with self.conn.cursor() as cursor:
            ...
            self.logger.info(
                f"Truncate successful: {cursor.rowcount:,} rows updated/inserted.\n"
            )
    ...
    def _delete_using_except(self, staging, mapping_dict: dict):
        ...
        for staging_field in staging.fields:
            ...
            staging_fields_composables.append(
                sql.SQL("STAGING.") + sql.Identifier(staging_field)
            )
    ...
    WHERE {where_composed}""").format(
        ...
        with self.conn.cursor() as cursor:
            self.logger.info(
                f"delete_using_statement:{cursor.mogrify(delete_using_stmt).decode()}"
            )
            cursor.execute(delete_using_stmt)
            self.logger.info(
                f"Delete Using Except statement successful: {cursor.rowcount:,} rows deleted.\n"
            )

    def _upsert_data_from_db(
        self, staging: "Postgres", mapping_dict: dict = {}, delete_stale: bool = False
    ):
        """
    ...
    """).format(
        ...
        with self.conn.cursor() as cursor:
            ...
            self.logger.info(
                f"Upsert Successful: {cursor.rowcount:,} rows updated/inserted.\n"
            )
    ...
    def _upsert_csv(self, mapping_dict: dict, delete_stale: bool):
        ...
        assert self.check_exists(self.temp_table_name, self.table_schema) == False, (
            f"Temporary Table {self.temp_table_name} already exists in this DB!"
        )
        ...
        staging = Postgres(
            connector=self.connector, table_name=self.temp_table_name, table_schema=None
        )
        self._upsert_data_from_db(
            staging=staging, mapping_dict=mapping_dict, delete_stale=delete_stale
        )
    ...
    def upsert(
        ...
        if self.primary_keys == set():
            raise ValueError(
                f'Upsert method requires that table "{self.fully_qualified_table_name}" have at least one column as primary key.'
            )
        mapping_dict = self._make_mapping_dict(column_mappings, mappings_file)
        ...
        if method == "csv":
            ...
            self._upsert_table(
                mapping_dict, staging_table, staging_schema, delete_stale
            )
        else:
